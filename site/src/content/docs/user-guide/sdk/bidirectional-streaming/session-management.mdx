...
`SnapshotSessionManager` is typed for `Agent` by default. Write
`SnapshotSessionManager[LocalAgent]` for a manager that attaches to a `BidiAgent`, or pass
a `snapshot_trigger` whose `agent_data` is annotated `LocalAgent` and the type is inferred.
